...
//
console.log("[open_call_list/search.js] CHIS契約情報検索")
...
var chis_contract_db = cloudant.db.use("open_call_list");
...
router.get("/", (req, res, next) => {
	...
	customer_number = req.query.customer_number;
	BO_Queue = req.query.BO_Queue;
	
	if (customer_number == undefined) {
		customer_number = "";
	}	

	if (BO_Queue == undefined) {
		BO_Queue = "";
	}	
	...
	switch (search_type) {
		...
		case "search": // progress = On Going, Delay & category != sysmod
			if (customer_number == "") {
				selector = '{ "BO_Queue": { "$eq": "' + BO_Queue + '" } }';
			} else if (BO_Queue == "") {
				selector = '{ "customer_number": "' + customer_number + '" }';
			} else {
				selector = '{ "$and": [ { "customer_number": { "$eq": "' + customer_number + '" } }, { "BO_Queue": { "$eq": "' + BO_Queue + '" } } ] }';
			}
			break;
/*
